...
def create_swagger_api(app):
    ...
    captcha_response_model = api.model(
        "CaptchaResponse",
        {
            "success": fields.Boolean(
                required=True, description="Request success status"
            ),
            "data": fields.Nested(
                api.model(
                    "CaptchaData",
                    {
                        "session_id": fields.String(
                            required=True, description="Unique session identifier"
                        ),
                        "captcha_image": fields.String(
                            required=True, description="Base64 encoded captcha image"
                        ),
                        "expires_in": fields.Integer(
                            required=True, description="Session expiry time in seconds"
                        ),
                    },
                )
            ),
    ...
    gst_details_request_model = api.model(
        "GSTDetailsRequest",
        {
            "session_id": fields.String(
                required=True,
                description="Session ID from captcha endpoint",
            ...
            "captcha": fields.String(
                required=True, description="Captcha text from image", example="ABC123"
            ),
        },
    )

    gst_services_request_model = api.model(
        "GSTServicesRequest",
        {
            "gstin": fields.String(
                required=True,
    ...
    gstin_validation_request_model = api.model(
        "GSTINValidationRequest",
        {
            "gstin": fields.String(
                required=True,
                description="GSTIN number to validate",
                example="24AAACC1206D1ZM",
            )
        },
    )
    ...
    error_response_model = api.model(
        ...
        {
            "success": fields.Boolean(
                required=True, description="Request success status (false)"
            ),
    ...
    health_response_model = api.model(
        "HealthResponse",
        {
            "status": fields.String(
                required=True, description="Health status", example="healthy"
            ),
            ...
            "response_time_ms": fields.Float(
                required=True, description="Response time in milliseconds"
            ),
            "active_sessions": fields.Integer(
                required=True, description="Number of active sessions"
            ),
        },
    )

    gstin_validation_response_model = api.model(
        "GSTINValidationResponse",
        {
            ...
            "data": fields.Nested(
                api.model(
                    ...
                    {
                        "gstin": fields.String(
                            required=True, description="Input GSTIN"
                        ),
                        "is_valid": fields.Boolean(
                            required=True, description="Validation result"
                        ),
                        "length": fields.Integer(
                            required=True, description="GSTIN length"
                        ),
                        "state_code": fields.String(
                            description="State code (first 2 digits)"
                        ),
                        "pan": fields.String(
                            description="PAN number (characters 3-12)"
                        ),
                        "entity_number": fields.String(
                            description="Entity number (13th character)"
                        ),
                        "default_z": fields.String(
                            description="Default Z character (14th character)"
                        ),
                        "check_digit": fields.String(
                            description="Check digit (15th character)"
                        ),
                    },
                )
            ),
