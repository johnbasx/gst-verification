...
class MetricsCollector:
    ...
    def _init_prometheus_metrics(self):
        ...
        # Application info
        self.app_info = Info(
            "gst_api_info", "Application information", registry=self.registry
        )
    ...
    def record_gstin_validation(self, is_valid: bool):
        ...
        if PROMETHEUS_AVAILABLE:
            self.gstin_validation_requests.labels(is_valid=str(is_valid).lower()).inc()
...
class AlertManager:
    ...
    def __init__(
        self, metrics_collector: MetricsCollector, health_checker: HealthChecker
    ):
        self.metrics = metrics_collector
    ...
    def _check_response_time_alerts(
    ...
    def _filter_cooldown_alerts(
        ...
        for alert in alerts:
            ...
            recent_alerts = [
                alert_time
                for alert_time in self.alert_history[alert_key]
            ...
            if not recent_alerts:
                ...
                self.alert_history[alert_key] = [
                    alert_time
                    for alert_time in self.alert_history[alert_key]
                    if (current_time - alert_time).total_seconds()
