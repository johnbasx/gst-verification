...
def setup_basic_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("logs/gst_api.log")],
    )
...
def create_app(config_name: Optional[str] = None) -> Flask:
    ...
    logger.info(
        f"GST Verification API initialized in {app.config.get('FLASK_ENV', 'unknown')} mode"
    )
...
def clean_expired_sessions() -> int:
    ...
    expired_sessions = [
        ...
        if current_time - session_data.get("created_at", current_time)
...
def register_routes(app: Flask):
    ...
    def get_captcha():
        ...
        try:
            ...
            response = requests_session.get(
                captcha_url, headers=headers, timeout=timeout, verify=True
            )
            ...
            logger.error(
                f"Connection error while fetching captcha for session: {session_id}"
            )
            return (
                jsonify(
                    {
    ...
    def get_gst_details():
        ...
        try:
            ...
            response = requests_session.post(
                search_url, json=payload, headers=headers, timeout=timeout, verify=True
            )
            ...
            if "error" in result or result.get("status") == "error":
                ...
                logger.warning(
                    f"GST portal error for GSTIN {gstin}: {error_message} (Code: {error_code})"
                )
            ...
            logger.error(
                f"Connection error while fetching GST details for GSTIN: {gstin}"
            )
            return (
                jsonify(
                    {
    ...
    def get_gst_services():
        ...
        try:
            ...
            response = session.get(
                services_url, headers=headers, timeout=timeout, verify=True
            )
            ...
            try:
                ...
                logger.error(
                    f"Invalid JSON response from GST portal services API: {str(e)}"
                )
                return (
                    jsonify(
                        {
            ...
            if "error" in result or result.get("status") == "error":
                ...
                logger.warning(
                    f"GST portal services error for GSTIN {gstin}: {error_message} (Code: {error_code})"
                )
                ...
                for activity in bzsdtls:
                    if isinstance(activity, dict):
                        activity_info = {
                            ...
                            "category": "Service"
                            if activity.get("saccd", "").startswith("99")
                            else "Goods",
                        }
            ...
            # Log successful retrieval
            logger.info(
                f"GST services details retrieved successfully for GSTIN: {gstin}"
            )
            ...
            logger.error(
                f"Connection error while fetching GST services for GSTIN: {gstin}"
            )
            return (
                jsonify(
                    {
    ...
    def validate_gstin_endpoint():
        ...
        try:
            ...
            gstin_info = {"gstin": gstin, "is_valid": is_valid, "length": len(gstin)}
...
if __name__ == "__main__":
    ...
    try:
        ...
        app_instance.run(host="0.0.0.0", port=port, debug=debug, threaded=True)
