...
class BaseConfig:
    ...
    RATE_LIMIT_CAPTCHA = int(os.environ.get('RATE_LIMIT_CAPTCHA', 20))  # requests per minute
    RATE_LIMIT_GST_DETAILS = int(os.environ.get('RATE_LIMIT_GST_DETAILS', 10))  # requests per minute
...
def validate_config(config_obj):
    ...
    secret_key = config_obj.get('SECRET_KEY') if hasattr(config_obj, 'get') else config_obj.SECRET_KEY
    if not secret_key or secret_key == 'dev-secret-key-change-in-production':
        flask_env = getattr(config_obj, 'FLASK_ENV', 'development') if hasattr(config_obj, 'FLASK_ENV') else 'development'
        if flask_env == 'production' or config_obj.__class__.__name__ == 'ProductionConfig':
